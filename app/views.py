...
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        print(record)
        if record == None:
            return Response({"message": "No User with Username"},
        ...
        if Insertedpassword == password:
            ...
            try:
                ...
                print(e)
            try:
                BITokenWeb = accessTokenBI['token']
            except Exception as e:
                print(e)
            try:
                BITokenMobile = accessTokenMobileBI['token']
            except Exception as e:
                print(e)
            # print("Abhishek")
            # print(accessTokenML)
            if len(record2) != 0:
                ...
                return Response(
                    {"username": username,
                     ...
                     "BIiframeWeb": record2[1],
                     "BIiframemobile": record2[15],
                     "MLiframe_web": record2[5],
                     "MLIframeMobile": record2[13],

                     "accesstokenBI": BITokenWeb,
...
def cloneReport(groupId, groupIdFromClone, APPLICATION_ID, APPLICATION_SECRET, USER_ID, USER_PASSWORD, reportId,
                reportName):
    body = {
        "name": reportName,
...
def PowerBiAutoMationAPI(name):
    ...
    records.append(api_response.json()["id"])
    json1 = {
        "name": "farrgut",
        "defaultMode": "Push",
        "tables": [
    ...
    reportIdWeb = "80a4d1dd-64e1-4963-bce6-0833704615a3"
    reportIdMobile = "f245b839-a05f-4700-b143-fb0071967119"
    print("Abhishek 1 Web")
    reportCloneWeb = cloneReport(api_response.json()["id"], groupIdForClone, APPLICATION_ID, APPLICATION_SECRET,
                                 USER_ID,
                                 USER_PASSWORD, reportIdWeb, "WebReport")
    print("Abhishek 2 Web")
    print(reportCloneWeb.json())
    records.append(reportCloneWeb.json()["id"])  # Adding report Id
    Abhishek = cloneReportRebind(api_response.json()["id"], reportCloneWeb.json()["id"], dataset.json()["id"],
    ...
    records.append("https://app.powerbi.com/reportEmbed?reportId=" + reportCloneWeb.json()[
        "id"] + "&autoAuth=true&ctid=d99798d5-98a3-4900-9e12-bcbce1aa8e27")  # Adding I Frame URL
    records.append(reportCloneWeb.json()['embedUrl'])
    # Mobile creation of Report Rebinding it and iframe generation and then embeded url generation done
    createReportCloneMobile = cloneReport(api_response.json()["id"], groupIdForClone, APPLICATION_ID,
                                          APPLICATION_SECRET, USER_ID,
                                          USER_PASSWORD, reportIdMobile, "MobileReport")
    records.append(createReportCloneMobile.json()["id"])  # report Id of mobile
    rebindResponse = cloneReportRebind(api_response.json()["id"], createReportCloneMobile.json()["id"],
                                       dataset.json()["id"],
                                       APPLICATION_ID, APPLICATION_SECRET, USER_ID, USER_PASSWORD)
    records.append("https://app.powerbi.com/reportEmbed?reportId=" + createReportCloneMobile.json()[
        "id"] + "&autoAuth=true&ctid=d99798d5-98a3-4900-9e12-bcbce1aa8e27")  # Adding I Frame URL mobile
    records.append(createReportCloneMobile.json()['embedUrl'])  # Adding Embeded Url Mobile
    print(len(records))
    print("After insertion of Mobilke data : ", records)
    return records
...
def InsertIntoDataBaseCountyDetails(records, username, encPassword, countyName, uniqueId):
	...
	unique_id, iframe, group_id, report_id, dataset_id, mliframe_web, embed_url_web_bi, group_id_ml, report_id_ml_mobile, dataset_id_ml, embed_url_mobile_ml, embed_url_web_ml, report_id_ml_web, mliframe_mobile,report_id_mobile_bi, iframe_mobile_bi, embeded_url_mobile_bi)
	VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s,%s,%s); '''
    record_to_insert2 = (
    uniqueId, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None)
...
def UpdateValueInDBForBI(uniqueId, records):
    ...
    dataset_id = %s,
    embed_url_web_bi = %s,
    report_id_mobile_bi = %s,
    iframe_mobile_bi = %s ,
    embeded_url_mobile_bi = %s
    WHERE unique_id = %s; '''
    record_to_insert1 = (
    records[3], records[0], records[2], records[1], records[4], records[5], records[6], records[7], uniqueId)
    cursor.execute(query1, record_to_insert1)
...
def GenerateTokens(uniqueId,choice):
    conn = psycopg2.connect(
        database="AristPOC", user='techment@aristanalyticspoc', password='Sanjeev@123',
        host='aristanalyticspoc.postgres.database.azure.com', port='5432'
    )
    ...
    schemaSet = "set schema 'tenant_details'"
    cursor.execute(schemaSet)
    query1 = "select * from tenant_details.test_tbl_county_details where unique_id = '" + uniqueId + "'"
    cursor.execute(query1)
    record2 = cursor.fetchone()
    conn.commit()
    conn.close()
    if( choice.lower()=='ml'):
        ...
        MLTokenMobile = None
        try:
            MLTokenWeb = accessTokenML['token']
        except Exception as e:
            print(e)
        try:
            MLTokenMobile = accessTokenMobileML['token']
    ...
    if(choice.lower()=='bi'):
        ...
        try:
            ...
            print(e)
        v=[BITokenWeb,BITokenMobile]
        return v
    else:
        return None

class AccessTokenForMLAndBI(APIView):
    def post(self,request):
        try:
            uniqueId = request.data["uniqueId"]
            choice = request.data["choice"]
            tokens = GenerateTokens(uniqueId, choice)
            if(tokens!= None):
                return Response({"WebToken": tokens[0],"MobileToken":tokens[1]}, status=status.HTTP_200_OK)
            else: return Response({"message": "Wrong Choice Selected "}, status=status.HTTP_400_BAD_REQUEST)
        except Exception as e:
            return Response({"message": str(e)}, status=status.HTTP_400_BAD_REQUEST)
