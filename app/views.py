...
def UpdateValueInDBForML(uniqueId, records):
    ...
    SET mliframe = %s
    WHERE unique_id = %s; '''
    record_to_insert1 = (records[0], uniqueId)
    cursor.execute(query1, record_to_insert1)
    query2 = ''' Update test_tbl_user
    set active = %s
    where county_unique_id = %s ;
...
def MLModel(countyName):
    ...
    for elt in cur.description:
        cols.append(elt[0])
    print(" Anshuman data transform started : 1701")
    ...
    print(" Anshuman data transform Completed : 1713")

    # Importing Data from source
    print(" Anshuman data Traning Model started : 1716")
    train = organizedData
    ...
    print(" Anshuman data Traning Model Done : 1734")
    print(" Anshuman data Pridiction Model Started : 1735")
    ...
    print(" Anshuman data Pridiction Model Done : 1752")
    print(" Anshuman data Data Manupulation Model Started : 1753")
    final = df_proj.reset_index()
    print("Anshuman error found " , final["index"].count())
    ...
    final['NUMBER_OF_SALES'] = final['NUMBER_OF_SALES'].fillna(0)
    final["NUMBER_OF_SALES"] = final["NUMBER_OF_SALES"] + final["Prediction"]
    final.drop("Prediction", axis='columns', inplace=True)
    print(" line 1775  ")

    average = (final.groupby(final.SALE_DATE.dt.year)['NUMBER_OF_SALES'].transform('mean')).tolist()
    ...
    print("Issue at line number 1792")
    final['Percent_Change_Per_Year'] = final['Percent_Change_Per_Year'].replace('0', None)
    print("Issue at line number 1794")
    final['NUMBER_OF_SALES'] = final['NUMBER_OF_SALES'].astype(int)
    print("Issue at line number 1796")
    final['NUMBER_OF_SALES'] = final['NUMBER_OF_SALES'].astype(str)
    print("Issue at line number 1798")
    final['forecast_lower'] = final['forecast_lower'].fillna(0).astype(np.int64, errors='ignore')
    print("Issue at line number 1800")
    final['forecast_upper'] = final['forecast_upper'].fillna(0).astype(np.int64, errors='ignore')
    print("Issue at line number 1802")
    final['forecast_upper'] = final['forecast_upper'].replace(0, None)
    print("Issue at line number 1804")
    final['forecast_lower'] = final['forecast_lower'].replace(0, None)
    print("Issue at line number 1806")
    final.rename(columns={'NUMBER_OF_SALES': 'SALE_PRICE'}, inplace=True)
    print("Issue at line number 1808")
    final = final.to_dict(orient='records')
    print(" Anshuman data Data Manupulation Model Done : 1810")

    print(" Anshuman data Report Creation Started : 1811")
    ...
    name = schema + '_WorkspaceX'
    print("Name of the report Schema : ",name)
    ...
    create_rowsInDataset(workspace_id, dataset_id, "rutherford", obj,APPLICATION_ID,APPLICATION_SECRET,USER_ID,USER_PASSWORD)
    ...
    iframe2 = "https://app.powerbi.com/reportEmbed?reportId=" + reportCloneMobile.json()[
        "id"] + "&autoAuth=true&ctid=d99798d5-98a3-4900-9e12-bcbce1aa8e27"
    print(" Anshuman data Report Creation Done : 2010")
    return iframe1
    ...
    for i in record:
        if (datetime.datetime.utcnow()) - timedelta(minutes=30) > i[4]:
            ...
            try:
                ...
                print("Exception is : ",e)
                MLIframe = "Error creating MLIframe " + str(e)
            print("Automated ML sucessfully Completed : ")
...
class CreationOfBI(APIView):
    ...
    def post(self, request):
        try:
            ...
            # print("Abhishek Zankyani")
            # print("With str function ",str(e))
            # print("Without str function ",e)
