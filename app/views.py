...
def generationAccesstoken(datasetId,reportId,groupId):
    ...
    # json2 = {
    #     {"datasets": [{"id": "a6899250-e1b0-4e32-a242-5367f1f01e65"}],
    #      "reports": [{"id": "97c109c6-1480-4f3f-938a-323428034067"}],
    #      "targetWorkspaces": [{"id": "23bf1831-3b80-4e18-bf14-9045eacef60d"}]}
    # }
...
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        if Insertedpassword == password:
            ...
            conn.commit()
            conn.close()
            accessToken = generationAccesstoken(datasetId=record2[4],reportId=record2[3],groupId=record2[2])
            if len(record2) == 7:
                return Response(
                    {"username": username, "unique_id": record2[0], "iframe": record2[1], "group_id": record2[2],
                     "report_id": record2[3], "dataset_id": record2[4],"MLiframe":record2[5],"accesstoken": accessToken['token'],"embeded_url":record2[6]}, status=status.HTTP_200_OK)
            else:
...
def InsertIntoDataBaseCountyDetails(records, username, encPassword, countyName, uniqueId):
    ...
    query2 = ''' INSERT INTO tenant_details.test_tbl_county_details(
    	unique_id, iframe, group_id, report_id, dataset_id, mliframe)
    	VALUES (%s, %s, %s, %s,%s,%s) '''
    record_to_insert2 = (uniqueId, None, None, None, None, None)
...
def UpdateValueInDB(uniqueId, records):
    # print()
    # print()
    # print()
    # print()
    ...
    embed_url = %s
    WHERE unique_id = %s; '''
    record_to_insert1 = (records[3], records[0], records[2], records[1], records[5],records[4], uniqueId)
    print(records[3], records[0], records[2], records[1], uniqueId)
    cursor.execute(query1, record_to_insert1)
...
def autoMl(countyName):
    ...
    df2 = df[["SALE_PRICE", "SALE_DATE"]]
    df2['SALE_PRICE'] = df2['SALE_PRICE'].astype('int')
    df2['SALE_DATE'] = pd.to_datetime(df2["SALE_DATE"])
    df2.index.freq = 'MS'
    df3 = df2[(df2['SALE_DATE'] > '2017-01-01') & (df2['SALE_DATE'] <= '2021-12-31')]
    df3 = df3.sort_values(by=['SALE_DATE'])
    df3 = df3.resample('MS', on='SALE_DATE').mean()

    # import matplotlib.pyplot as plt
    # df3.plot(figsize=(12,6))

    # from statsmodels.tsa.seasonal import seasonal_decompose
    # results = seasonal_decompose(df3['SALE_PRICE'])
    # results.plot();
    ...
    final = df_proj.reset_index()

    final.rename(columns={'index': 'SALE_DATE'}, inplace=True)
    ...
    final['Years'] = final['SALE_DATE'].dt.to_period('Y')

    final['Prediction'] = final['Prediction'].fillna(0)
    final['SALE_PRICE'] = final['SALE_PRICE'].fillna(0)
    final["SALE_PRICE"] = final["SALE_PRICE"] + final["Prediction"]
    final.drop("Prediction", axis='columns', inplace=True)

    average = (final.groupby(final.SALE_DATE.dt.year)['SALE_PRICE'].transform('mean')).tolist()
    av = pd.DataFrame(average)
    av[0] = av[0].pct_change() * 100
    av[0] = av[0].fillna(0).astype(np.int64, errors='ignore')
    final['Percent_Change_Per_Year'] = av

    final['Years'] = final['Years'].astype(str)
    final['Quarters'] = final['SALE_DATE'].dt.quarter
    final['SALE_DATE'] = final['SALE_DATE'].astype(str)
    final['Quarters'] = final['Quarters'].astype(str)

    final['Percent_Change_Per_Year'] = final['Percent_Change_Per_Year'].astype(str)
    final['Percent_Change_Per_Year'] = final['Percent_Change_Per_Year'].replace('0', None)
...
#     final['forecast_lower'] = final['forecast_lower'].replace(0, None)
    ...
    RESOURCE = "https://analysis.windows.net/powerbi/api"  # Don't change that.
    APPLICATION_ID = "15bb3348-c575-4b34-a87f-f915212cefb3"  # The ID of the application in Active Directory
    APPLICATION_SECRET = "3ae8Q~aVlWbeRj1AKUCAwW8B76K57723bBvNdcM4"  # A valid key for that application in Active Directory

    # In[5]:

    USER_ID = "arpitmasih77@37bs87.onmicrosoft.com"  # A user that has access to PowerBI and the application
    USER_PASSWORD = "Arpit@techment123"  # The password for that user

    # In[6]:

    # GROUP_ID = 'bcf02903-f5c1-4150-bd04-c156e8a19257'  # The id of the workspace containing the report you want to embed
    ...
    def create_rowsInDataset(groupId, datasetId, tableName, body):
    ...
    dataset = create_dataset(workspace_id, json)
    dataset.json()
    ...
    # with open('rutherford.csv') as file_object:
    ...
    #             i[3] = None
    #         if  type(i[2]) == str :
    #             i[2] = int(math.floor(float(i[2])))
    #         if type(i[3]) == str:
    #              i[3] = int(math.floor(float(i[3])))
    ...
    create_rowsInDataset(workspace_id, dataset_id, "rutherford", obj)

    # In[68]:

    target_group_id = workspace_id
    source_group_id = "414c7de8-33cc-4b98-b232-13eabcc2e501"
    source_report_id = "c7420d9a-bcaf-496d-b1b0-8367fafe5e1f"
    reportClone = cloneReport(target_group_id, source_group_id, source_report_id)
    print(reportClone.json())

    # In[69]:

    cloneReportRebind(workspace_id, reportClone.json()["id"], dataset.json()["id"])

    return "https://app.powerbi.com/reportEmbed?reportId=" + reportClone.json()[
        "id"] + "&autoAuth=true&ctid=fed92f24-32f1-45ae-9885-03d383f5db72"

    # In[ ]:

def AutomationOfBIAndML():
    conn = psycopg2.connect(
        database="AristPOC", user='techment@aristanalyticspoc', password='Sanjeev@123',
        host='aristanalyticspoc.postgres.database.azure.com', port='5432'
    )
    print("testrun3")
    print((datetime.datetime.utcnow()) - timedelta(minutes=15))
    ...
    print(record)
    for i in record:
        print((datetime.datetime.utcnow()) - timedelta(minutes=30) > i[4])
        if (datetime.datetime.utcnow()) - timedelta(minutes=30) > i[4]:
            ...
            try:
                ...
                MLIframe = "error in creating MLIframe"
            print("Automated ML sucessfully Completed : ")

            # print(MLIframe)
            details.append(MLIframe)
            print(details)
            UpdateValueInDB(i[0], details)
    conn.commit()
    conn.close()


class CreationOfBIAndML(APIView):
    ...
    def post(self, request):
        try:
            ...
            print("Issue Occured In exception Block")
            return Response({"message": "Failed to register"}, status=status.HTTP_400_BAD_REQUEST)
